...
mod tests {
    ...
    #[test]
    fn list_identity_fails_if_identity_instance_is_missing() {
        let mut registry = Registry::new();
        ...
        let mut buffer = BytesMut::new();
        let result = list_identity(&registry, &mut buffer);
        assert!(result.is_err());
    }

    #[test]
    fn list_identity_fails_if_tcp_ip_instance_is_missing() {
        let mut registry = Registry::new();
